...
export const AnnotationsProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const setEpisode = useCallback(
    (
      ...
      // Hydrate precedence: unsaved session edits > saved sidecar JSON >
      // shipped parquet atoms. Read session synchronously first so in-progress
      // work survives an episode toggle.
      let sessionAtoms: LanguageAtom[] | null = null;
      try {
        const raw = sessionStorage.getItem(
          storageKey(identKey(newIdent), newEpisodeId),
        );
        if (raw) sessionAtoms = JSON.parse(raw) as LanguageAtom[];
      } catch {
        /* ignore */
      }
      const hasSessionEdits = !!sessionAtoms && sessionAtoms.length > 0;
      const initial = hasSessionEdits
        ? (sessionAtoms as LanguageAtom[])
        : (initialAtoms ?? []);
      setAtoms(initial);
      savedSnapshotRef.current = JSON.stringify(initial);
      setDirty(false);
      // Frame timestamps always come from the parquet (no backend dependency).
      setFrameTimestamps(initialFrameTimestamps ?? []);

      // Read the saved annotation sidecar (meta/lerobot_annotations.json).
      // It reflects the latest explicit save, so it overrides the shipped
      // parquet atoms — but never clobbers unsaved session edits.
      if (!hasSessionEdits) {
        fetchEpisodeAtoms(newEpisodeId, newIdent)
          .then((savedAtoms) => {
            if (savedAtoms && savedAtoms.length > 0) {
              setAtoms(savedAtoms);
              savedSnapshotRef.current = JSON.stringify(savedAtoms);
              setDirty(false);
            }
          })
          .catch(() => {
            /* no sidecar yet — keep parquet/session atoms */
          });
      }
    },
    [],
